...
package object nodescala {
  ...
  implicit class FutureCompanionOps(val f: Future.type) extends AnyVal {
    ...
    def always[T](value: T): Future[T] = {
      ...
      Future {
        ...
        case Success(res) => p.complete(Try(res))
        case Failure(exp) => p.complete(Try(value))
      }
      p.future
    }
    ...
    def run()(f: CancellationToken => Future[Unit]): Subscription = {
      ...
      val sub = new Subscription {
        def unsubscribe(): Unit = tok.unsubscribe()
      }

      val res = f(tok.cancellationToken)
      res.onComplete{row =>
        sub.unsubscribe()
      }
      sub
  ...
  implicit class FutureOps[T](val f: Future[T]) extends AnyVal {
    ...
    def continueWith[S](cont: Future[T] => S): Future[S] = {
      val ps = Promise[S]()
      f.onComplete {
        case Success(tRes) => ps.complete(Try(cont(f)))
        case Failure(err) => ps.failure(err)
      }
      ps.future
    }
